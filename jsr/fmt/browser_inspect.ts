...
function quote(value: string, limit?: number, escapeSequences = true): string {
  if (limit !== undefined && value.length > limit) value = value.slice(0, limit) + "...";
  let result = "";
...
function key(value: PropertyKey): string {
 ...
 */
export function inspectBrowser(value: unknown, options: InspectOptions = {}): string {
  const active = new WeakSet<object>();
  const depth = options.depth ?? 4;
  const limit = options.iterableLimit ?? 100;
  const indent = "  ";

  const render = (current: unknown, remaining: number, level: number): string => {
    ...
    if (remaining < 0) {
      ...
      if (current instanceof Map) return "[Map]";
    ...
    try {
      ...
      if (Array.isArray(current)) {
        ...
        if (empty) entries.push(`<${empty} empty item${empty === 1 ? "" : "s"}>`);
        if (current.length > limit) entries.push(`... ${current.length - limit} more item(s)`);
      ...
      const entries = properties.slice(0, limit).map((property) => {
        ...
        if (!descriptor) propertyValue = "undefined";
      ...
      if (properties.length > limit) entries.push(`... ${properties.length - limit} more item(s)`);
