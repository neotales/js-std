...
test("fmt::bytes rejects invalid input", () => {
  throws(() => format("" as never), TypeError);
  throws(() => format("1" as never), TypeError);
  throws(() => format(true as never), TypeError);
  throws(() => format(null as never), TypeError);
...
test("fmt::bytes supports fractional digit options", () => {
  ...
  equal(
    format(32_768, { minimumFractionDigits: 2, maximumFractionDigits: 3, binary: true }),
    `32${decimal}00 kiB`,
  );
});
