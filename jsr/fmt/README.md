...
The `inspect` function uses the runtime-native inspector in Deno, Node.js, and Bun. In browsers,
it uses a Node-like fallback that supports primitives, objects, arrays, maps, sets, circular references,
and `depth`, `compact`, `breakLength`, `escapeSequences`, `iterableLimit`, `sorted`, `trailingComma`,
`strAbbreviateSize`, `showHidden`, and `getters` options. Browser color and proxy inspection are deferred
to the ANSI module.
...
The byte and duration formatters are available from focused subpaths to avoid a root-export name collision.

```typescript
import { format as formatBytes } from "@neotales/fmt/bytes";
import { format as formatDuration } from "@neotales/fmt/duration";

formatBytes(1337); // "1.34 kB"
formatBytes(1025, { binary: true }); // "1 kiB"
formatDuration(99_674, { ignoreZero: true }); // "1m 39s 674ms"
formatDuration(99_674, { style: "full", ignoreZero: true });
...
For inspect or the `/I` flag inspect uses the native runtime inspector where available and a
browser-safe fallback otherwise. Exact formatting can vary between runtimes.
