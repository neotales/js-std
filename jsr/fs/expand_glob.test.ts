...
test("expandGlobSync includes literal paths and can exclude directories", () => {
  withTestRootSync((root) => {
    ...
    writeTextFileSync(join(root, "nested", "child.ts"), "");
    deepStrictEqual(
      [...expandGlobSync("root.ts", { root })].map(({ path }) => relative(root, path)),
      ["root.ts"],
    );
    deepStrictEqual(
...
test("expandGlob supports parent patterns and extended matching", async () => {
  await withTestRoot(async (root) => {
    ...
    deepStrictEqual(
      (await Array.fromAsync(expandGlob("nested/../*.ts", { root })))
        .map(({ path }) => relative(root, path))
        .sort(),
      ["one.ts", "two.ts"],
    );
    deepStrictEqual(
      (await Array.fromAsync(expandGlob("@(one|two).ts", { extended: true, root })))
        .map(({ path }) => relative(root, path))
        .sort(),
      ["one.ts", "two.ts"],
    );
...
test("expandGlobSync supports parent patterns and extended matching", () => {
  withTestRootSync((root) => {
    ...
    deepStrictEqual(
      [...expandGlobSync("nested/../*.ts", { root })]
        .map(({ path }) => relative(root, path))
        .sort(),
      ["one.ts", "two.ts"],
    );
    deepStrictEqual(
      [...expandGlobSync("@(one|two).ts", { extended: true, root })]
        .map(({ path }) => relative(root, path))
        .sort(),
      ["one.ts", "two.ts"],
    );
...
test("expandGlob honors each extended operator with generated names", async () => {
  await withTestRoot(async (root) => {
    ...
    const paths = async (pattern: string) =>
      (await Array.fromAsync(expandGlob(pattern, { extended: true, root })))
        .map(({ path }) => relative(root, path))
        .sort();
...
test("expandGlobSync honors each extended operator with generated names", () => {
  withTestRootSync((root) => {
    ...
    const paths = (pattern: string) =>
      [...expandGlobSync(pattern, { extended: true, root })]
        .map(({ path }) => relative(root, path))
        .sort();
  ...
  await withTestRoot(async (root) => {
    ...
    deepStrictEqual(
      (await Array.fromAsync(expandGlob("nested/../**/*.ts", { root })))
        .map(({ path }) => relative(root, path))
        .sort(),
      ["literal.ts", join("narrow", "inside.ts"), "outside.ts"],
    );
    deepStrictEqual(
      await Array.fromAsync(expandGlob("literal.ts", { exclude: ["literal.ts"], root })),
      [],
    );
    deepStrictEqual(
...
test("expandGlobSync handles trailing directories, parent globstars, excludes, and narrowed roots", () => {
  withTestRootSync((root) => {
    ...
    deepStrictEqual(
      [...expandGlobSync("directories/*/", { root })].map(({ path }) => relative(root, path)),
      [join("directories", "child")],
    );
    deepStrictEqual(
      [...expandGlobSync("nested/../**/*.ts", { root })]
    ...
    deepStrictEqual(
      [...expandGlobSync("**/*.ts", { root: narrow })].map(({ path }) => relative(narrow, path)),
      ["inside.ts"],
    );
  });
});
...
test("expandGlobSync accepts a generated symlink as its root", { skip: isWindows }, () => {
  withTestRootSync((root) => {
    ...
    deepStrictEqual(
      [...expandGlobSync("**/*.ts", { root: link })].map(({ path }) => relative(link, path)),
      ["child.ts"],
    );
  });
});
...
test(
  "expandGlob follows generated links with and without canonical paths",
  { skip: isWindows },
  async () => {
    await withTestRoot(async (root) => {
      const target = join(root, "target");
...
test(
  "expandGlobSync follows generated links with and without canonical paths",
  { skip: isWindows },
  () => {
    withTestRootSync((root) => {
      const target = join(root, "target");
