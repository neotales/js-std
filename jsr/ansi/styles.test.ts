...
function withColors(callback: () => void): void {
...
test("ansi::rgb styles clamp and convert colors", () => {
  withColors(() => {
    ...
    equal(
      rgb24("x", { r: Number.NaN, g: Number.POSITIVE_INFINITY, b: Number.NEGATIVE_INFINITY }),
      "\x1b[38;2;0;0;0mx\x1b[39m",
    );
  });
});
...
test("ansi::links follow settings and strip OSC-8 sequences", () => {
  ...
  try {
    equal(
      link("docs", "https://example.com/\x1b[31m"),
      "\x1b]8;;https://example.com/[31m\x1b\\docs\x1b]8;;\x1b\\",
    );
...
test("ansi::settings and public module exports work", async () => {
  const ansi = await import("./mod.ts");
