...
const result = await cmd(["ls", "-la"]).withCwd("/tmp").output();
...
| Function                                       | Description                                              |
| ---------------------------------------------- | -------------------------------------------------------- |
| `which(fileName, prependPath?, useCache?)`     | Finds the first executable path asynchronously           |
| `whichSync(fileName, prependPath?, useCache?)` | Finds the first executable path synchronously            |
| `whichAll(fileName, prependPath?)`             | Finds every executable path or glob match asynchronously |
| `whichAllSync(fileName, prependPath?)`         | Finds every executable path or glob match synchronously  |
...
// Find every PATH match, including glob matches
...
`ArgsBuilder` constructs command arguments without shell interpolation.
...
const args = new ArgsBuilder({ assign: "=" })
  ...
  .postArgs("--amend")
  .build();
// ["git", "commit", "--verbose", "--message=\"release notes\"", "--", "--amend"]
```
...
const piped = await cmd(["echo", "hello world"]).pipe(["grep", "hello"]).pipe("cat").output();
