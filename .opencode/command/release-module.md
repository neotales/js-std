...
5. Commit the release preparation, push `dev`, then create and push the tag with `SSH_AUTH_SOCK=/home/dev/.ssh/agent/s.9ShwXHKxSA.agent.fnSG70oDVh`. Use `mise exec -- gh` to monitor the branch CI and tag-triggered release workflow until completed successfully. Approve the release environment when GitHub requests it.
