...
test("path::posix.fromFileUrl()", function () {
    ...
    throws(() => posix.fromFileUrl("http://localhost/foo"), TypeError, 'URL must be a file URL: received "http:"');
    throws(() => posix.fromFileUrl("abcd://localhost/foo"), TypeError, 'URL must be a file URL: received "abcd:"');
});
test("path::windows.fromFileUrl()", function () {
    ...
    throws(() => windows.fromFileUrl("http://localhost/foo"), TypeError, 'URL must be a file URL: received "http:"');
    throws(() => windows.fromFileUrl("abcd://localhost/foo"), TypeError, 'URL must be a file URL: received "abcd:"');
});
