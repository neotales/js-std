...
import { expandGlob, expandGlobSync, isdir, isdirSync, isfile, isfileSync, readdir, readdirSync, } from "@neotales/fs";
...
export function whichSync(fileName, prependPath, useCache = true, debug = false) {
    ...
    for (const pathSegment of pathSegments) {
        ...
        if (WIN) {
            const hasPathExt = pathExtSegments.find((segment) => endsWithFold(fileName, segment)) !== undefined;
            if (!hasPathExt) {
                try {
                    let first;
 ...
 * ```
 */
export async function whichAll(fileName, prependPath, debug = false) {
    if (isNullOrSpace(fileName)) {
        throw new Error("Argument 'fileName' cannot be null or empty.");
    }
    if (isAbsolute(fileName)) {
        if (!isGlob(fileName))
            return (await isfile(fileName)) ? [fileName] : [];
        const paths = [];
    ...
    const seen = new Set();
    for (const pathSegment of pathSegments) {
        if (isNullOrEmpty(pathSegment) || !(await isdir(pathSegment)))
            continue;
        try {
            for await (const entry of readdir(pathSegment)) {
 ...
 * ```
 */
export function whichAllSync(fileName, prependPath, debug = false) {
    if (isNullOrSpace(fileName)) {
        throw new Error("Argument 'fileName' cannot be null or empty.");
    }
    if (isAbsolute(fileName)) {
        if (!isGlob(fileName))
            return isfileSync(fileName) ? [fileName] : [];
        const paths = [];
