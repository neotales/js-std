...
import { deepStrictEqual as equal, fail, notDeepStrictEqual as notEqual, ok, throws, } from "node:assert/strict";
const nope = (value, message) => ok(!value, message);
import { test } from "node:test";
import { Command, exec, execSync, runSync, ShellCommand, spawn, } from "./command.js";
...
test("exec::Command - set cwd", async (t) => {
    ...
    let dir = globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).dirname;
    dir ??= dirname(fromFileUrl(globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).url));
...
test("exec::Command - outputSync method", { skip: !echo }, (t) => {
    ...
    equal(output.code, 0, `exit code was ${output.code} and should be 0. Stdout: ${output.text()} Stderr: ${output.errorText()}`);
});
