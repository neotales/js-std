...
function addAlias(aliases, key, values) {
    ...
    for (const name of all) {
        aliases.set(name, all.filter((item) => item !== name));
    }
}
...
function coerceValue(name, value, options) {
    if (typeof value === "boolean" || hasName(name, options.string))
        return value;
 ...
 */
export function parse(argsOrOptions, maybeOptions = {}) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg.startsWith("--")) {
            ...
            else if (hasName(name, options.boolean) ||
                args[i + 1] === undefined ||
                isOption(args[i + 1])) {
        ...
        else if (hasName(short, options.boolean) ||
            args[i + 1] === undefined ||
            isOption(args[i + 1])) {
