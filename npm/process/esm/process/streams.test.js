...
const stringIncludes = (value, expected, message) => ok(value.includes(expected), message);
// @deno-ig
...
import { basename, dirname, fromFileUrl } from "../path/mod.js";
import { spawn, spawnSync } from "node:child_process";
const sourceDir = dirname(fromFileUrl(globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).url));
...
const writeStdoutScript = 'import { stdout } from "./streams.js"; stdout.writeSync(new TextEncoder().encode("writeSync\\n")); await stdout.write(new TextEncoder().encode("write\\n"));';
const writeStderrScript = 'import { stderr } from "./streams.js"; stderr.writeSync(new TextEncoder().encode("writeSync\\n")); await stderr.write(new TextEncoder().encode("write\\n"));';
const readStdinScript = 'import { stdin, stdout } from "./streams.js"; const buffer = new Uint8Array(1024); while (true) { const count = stdin.readSync(buffer); if (count === null) break; if (count > 0) stdout.writeSync(buffer.subarray(0, count)); }';
const readStdinAsyncScript = 'import { stdin, stdout } from "./streams.js"; const buffer = new Uint8Array(1024); while (true) { const count = await stdin.read(buffer); if (count === null) break; if (count > 0) await stdout.write(buffer.subarray(0, count)); }';
test("process::stdout", () => {
    if (DENO) {
