...
test("walk returns exactly its empty generated root", async () => {
    await withTestRoot(async (root) => {
        deepStrictEqual((await Array.fromAsync(walk(root))).map(({ path }) => path), [root]);
    });
});
test("walkSync returns exactly its empty generated root", () => {
    withTestRootSync((root) => {
        deepStrictEqual([...walkSync(root)].map(({ path }) => path), [root]);
    });
});
test("walkSync observes generated depth limits", () => {
    withTestRootSync((root) => {
        ...
        deepStrictEqual([...walkSync(root, { maxDepth: 1 })].map(({ path }) => relative(root, path) || ".").sort(), [".", "nested"]);
    });
});
...
test("walkSync supports extension and file/directory filters", () => {
    withTestRootSync((root) => {
        ...
        deepStrictEqual([...walkSync(root, { exts: ["ts"], includeDirs: false })]
            .map(({ path }) => relative(root, path))
            .sort(), ["root.ts"]);
        deepStrictEqual([...walkSync(root, { includeFiles: false })]
            .map(({ path }) => relative(root, path) || ".")
            .sort(), [".", "nested"]);
...
test("walk excludes generated files when requested", async () => {
    await withTestRoot(async (root) => {
        ...
        deepStrictEqual((await Array.fromAsync(walk(root, { includeFiles: false })))
            .map(({ path }) => relative(root, path) || ".")
            .sort(), [".", "nested"]);
    });
});
test("walk accepts generated extension variants", async () => {
    await withTestRoot(async (root) => {
        ...
        for (const extension of ["ts", ".ts"]) {
            deepStrictEqual((await Array.fromAsync(walk(root, { exts: [extension], includeDirs: false }))).map(({ path }) => relative(root, path)), ["file.ts"]);
        }
    });
});
test("walkSync skips generated paths matching a regular expression", () => {
    withTestRootSync((root) => {
        ...
        deepStrictEqual([...walkSync(root, { includeDirs: false, skip: [/skip/] })].map(({ path }) => relative(root, path)), ["keep.ts"]);
    });
});
test("walk follows generated directory links only when requested", { skip: isWindows }, async () => {
    await withTestRoot(async (root) => {
        ...
        const following = await Array.fromAsync(walk(root, { followSymlinks: true, canonicalize: false }));
        strictEqual(withoutFollowing.some(({ path }) => path === join(link, "child")), false);
        strictEqual(following.some(({ path }) => path === join(link, "child")), true);
        strictEqual(withoutFollowing.some(({ path }) => path === link), true);
        strictEqual(withoutFollowing.find(({ path }) => path === link)?.isSymlink, true);
        strictEqual(canonical.some(({ path }) => path === join(link, "child")), false);
    });
});
...
test("walk supports period-prefixed and regular-expression extensions", async () => {
    await withTestRoot(async (root) => {
        ...
        deepStrictEqual((await Array.fromAsync(walk(root, { match: [/\.ts$/], includeDirs: false })))
            .map(({ path }) => relative(root, path))
            .sort(), ["root.ts"]);
    });
});
test("walkSync supports period-prefixed and regular-expression extensions", () => {
    withTestRootSync((root) => {
        ...
        deepStrictEqual([...walkSync(root, { exts: [".ts"], includeDirs: false })]
            .map(({ path }) => relative(root, path))
            .sort(), ["root.ts"]);
        deepStrictEqual([...walkSync(root, { match: [/\.md$/], includeDirs: false })]
            .map(({ path }) => relative(root, path))
            .sort(), [join("nested", "child.md")]);
    });
});
test("walk rejects when its generated root is removed during iteration", async () => {
    await withTestRoot(async (root) => {
        ...
        await rejects(Array.fromAsync(walk(directory), async () => await rm(directory, { recursive: true })));
    });
});
