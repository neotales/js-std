...
export async function mkdtemp(options) {
    if (isDeno) {
        ...
        try {
            const { mkdtemp: nodeMkdtemp, rename } = getNodeFs().promises;
            const { tmpdir } = getNodeOs();
            const { join, sep } = getNodePath();
            if (!options) {
                return await nodeMkdtemp(join(tmpdir(), sep));
            }
...
export function mkdtempSync(options) {
    if (isDeno) {
        ...
        const { dir = undefined, prefix = undefined, suffix = undefined } = options ?? {};
        try {
            const { mkdtempSync: nodeMkdtempSync, renameSync } = getNodeFs();
            const { tmpdir } = getNodeOs();
            const { join, sep } = getNodePath();
            if (!options) {
                return nodeMkdtempSync(join(tmpdir(), sep));
            }
            let prependPath = tmpdir();
            if (dir != null) {
                prependPath = typeof dir === "string" ? dir : ".";
            ...
            if (suffix != null && typeof suffix === "string") {
                const tempPath = nodeMkdtempSync(prependPath);
                const combinedTempPath = "".concat(tempPath, suffix);
                renameSync(tempPath, combinedTempPath);
                return combinedTempPath;
            }
            return nodeMkdtempSync(prependPath);
        }
        catch (error) {
            throw mapError(error);
        }
