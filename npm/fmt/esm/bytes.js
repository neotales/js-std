...
export function format(num, options = {}) {
    ...
    if (num < 1)
        return prefix + toLocaleString(num, options.locale, localeOptions) + " " + units[0];
    const exponent = Math.min(Math.floor(options.binary ? Math.log(num) / Math.log(1024) : Math.log10(num) / 3), units.length - 1);
