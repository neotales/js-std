...
test("fs::Deno permission boundaries reject denied ensure and glob reads", { skip: !deno }, async () => {
    await withTestRoot(async (root) => {
        ...
        await deniesRead(root, "./expand_glob.ts", "expandGlob", "for await (const _ of expandGlob($TARGET)) {} ");
    });
});
