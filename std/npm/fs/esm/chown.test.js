...
test("chown rejects changing ownership when the runtime user is deterministically unprivileged", { skip: isWindows || uid() === null || uid() === 0 }, async () => {
    await withTestRoot(async (root) => {
        const file = join(root, "file");
        const currentUid = uid();
        await writeTextFile(file, "value");
...
test("chownSync rejects changing ownership when the runtime user is deterministically unprivileged", { skip: isWindows || uid() === null || uid() === 0 }, () => {
    withTestRootSync((root) => {
        const file = join(root, "file");
        const currentUid = uid();
        writeTextFileSync(file, "value");
