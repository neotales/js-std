...
test("results::ok and fail expose their state", () => {
  ...
  throws(() => fail<number>(new Error("boom")).orThrow(), ResultError);

  const success: Result<number, string> = ok(2);
  if (success.ok()) strictEqual(success.value.toFixed(), "2");
  const failure: Result<number, string> = fail("bad");
  if (failure.failed()) strictEqual(failure.error.toUpperCase(), "BAD");
});
...
test("results::fallback, match, and inspect helpers are lazy", () => {
  let calls = 0;
  equal(
    fail<number, string>("bad").orDefault(() => ++calls),
    1,
  );
  equal(
    ok(2).orDefault(() => ++calls),
    2,
  );
  equal(
    match(
  ...
  equal(
    fail<number, string>("bad").mapError((error) => error.length, 0),
    3,
  );
  equal(calls, 1);
});
...
test("results::toPromise recovers only when a rejection handler is provided", async () => {
  ...
  strictEqual(await fail<number, string>("bad").toPromise(undefined, (error) => error.length), 3);
  await rejects(() => fail<number, string>("bad").toPromise());
});

test("results::constructors and aggregators retain values and errors", async () => {
  strictEqual(fromNullable("id", "missing").value, "id");
  strictEqual(fromNullable<string | undefined, string>(undefined, "missing").error, "missing");
  ...
  strictEqual(all([ok(1), fail<number, string>("bad")]).error, "bad");
  strictEqual(
    (await allAsync([Promise.resolve(ok(1)), Promise.resolve(ok(2))])).value?.join(","),
    "1,2",
  );
