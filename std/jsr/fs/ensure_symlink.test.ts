...
test(
  "ensureSymlink accepts URLs, relative targets, and detects conflicts",
  { skip: isWindows },
  async () => {
    await withTestRoot(async (root) => {
...
test(
  "ensureSymlink creates directory and relative generated links",
  { skip: isWindows },
  async () => {
    await withTestRoot(async (root) => {
...
test(
  "ensureSymlinkSync creates directory and relative generated links",
  { skip: isWindows },
  () => {
    withTestRootSync((root) => {
      const directory = join(root, "directory");
...
test(
  "ensureSymlink rejects every occupied destination without changing it",
  { skip: isWindows },
  async () => {
    await withTestRoot(async (root) => {
      const target = join(root, "target");
...
test(
  "ensureSymlinkSync rejects every occupied destination without changing it",
  { skip: isWindows },
  () => {
    withTestRootSync((root) => {
      const target = join(root, "target");
