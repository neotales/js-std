...
test("writeTextFile accepts an active signal and exposes abort names and reasons", async () => {
  await withTestRoot(async (root) => {
    ...
    await rejects(
      writeTextFile(join(root, "aborted"), "value", { signal: aborted.signal }),
      (error) => {
        strictEqual((error as Error).name, "AbortError");
        return true;
      },
    );
    ...
    await rejects(
      writeTextFile(join(root, "reason"), "value", { signal: withReason.signal }),
      (error) => {
        strictEqual(
          error === reason || (error as Error & { cause?: unknown }).cause === reason,
          true,
        );
        return true;
      },
    ...
    await rejects(
      writeTextFile(join(root, "primitive-reason"), "value", { signal: primitive.signal }),
      (error) => {
        strictEqual(
          error === primitiveReason ||
            (error as Error & { cause?: unknown }).cause === primitiveReason,
          true,
        );
        return true;
      },
