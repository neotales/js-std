...
function forcedMode(value: string): AnsiMode {
  switch (value.toLowerCase()) {
    ...
    default:
      return AnsiModes.toValue(value) as AnsiMode;
  }
}

function detectCi(hasValue: (name: string) => boolean): AnsiMode | undefined {
  if (!hasValue("CI")) return undefined;
 ...
 */
export function detectMode(options: DetectOptions = {}): AnsiMode {
  const values = options.env;
  const getValue = (name: string): string | undefined => (values ? values[name] : get(name));
  const hasValue = (name: string): boolean => (values ? values[name] !== undefined : has(name));
  ...
  if (hasValue("NO_COLOR")) return AnsiModes.None;

  const forceColor = getValue("FORCE_COLOR");
  if (forceColor !== undefined && forceColor !== "") return forcedMode(forceColor);
  const color = getValue("COLOR") || getValue("ANSI_COLORS") || getValue("BEARZ_ANSI_COLOR");
  if (color !== undefined && color !== "") return AnsiModes.toValue(color) as AnsiMode;

  const ci = detectCi(hasValue);
  if (ci !== undefined) return ci;
  if (getValue("COLORTERM") === "truecolor") return AnsiModes.TwentyFourBit;
